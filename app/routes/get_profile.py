...
def get_profile():
    ...
    try:
        logger.info("[get] Validating request args...")
    ...
    try:
        ...
        logger.info(f"[get] Located {realm}")
    ...
    try:
        logger.debug(f"[get] Finding account ({realm.id}, {player.hash})...")
        ...
        logger.info(f"[get] Account ({realm.id}, {player.hash}) not found, "
                    f"creating new account for '{username}' in '{realm_name}'...")
        account = Account(realm_id=realm.id, player_hash=player.hash, world_id=realm.world_id)
        db.session.add(account)
        db.session.commit()
